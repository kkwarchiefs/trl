...
senti_model = AutoModelForSequenceClassification.from_pretrained('/search/ai/pretrain_models/roberta-base-finetuned-jd-binary-chinese/')
sentiment_pipe = pipeline('sentiment-analysis', model=senti_model, tokenizer=senti_tokenizer, device=ppo_trainer.model.device)
...
for cur_big_epoch in range(10):
    for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
        ...
        stats = ppo_trainer.step([torch.tensor(r).squeeze() for r in query_input_ids_tensors], response_tensors, rewards, RM_batch[-2], RM_batch[-1])
        if str(ppo_trainer.accelerator.device) == "cuda:0":
            print(str(ppo_trainer.accelerator.device))
